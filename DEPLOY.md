...
| `PWA_API_KEY`, `PWA_TEAM_UUID`, `PWA_TEAMATE_UUID` | источник PWApartners (пусто — источник не подключается) |
...
зарегистрированы и не стоят на паузе:

```bash
docker compose logs bot | grep job.registered   # next_run_time не должен быть None
```

В боте то же самое показывает `/jobs` — расписание, время следующего запуска и
сколько доменов сейчас в очереди на проверку.
