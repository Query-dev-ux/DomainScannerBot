...
PWA-платформ (PWA.partners, SkakApp), проверяет их в security-сервисах, хранит историю
...
|---|---|---|
| PWA.partners | `PWA_API_KEY`, `PWA_TEAM_UUID`, `PWA_TEAMATE_UUID` | `GET /dash_api/domains/list` |
...
из источника, перестаёт проверяться — в отчёте синхронизации это «удалено из источника»;
...
минималистичный: без эмодзи, иерархия — жирный заголовок, домен моноширинным, служебное
курсивом; в карточке проверки жирным выделены только проверки, которые что-то нашли.
Время в сообщениях — в часовом поясе `DISPLAY_TIMEZONE` (по умолчанию UTC).
