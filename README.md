...
**Проверяется каждый домен, который отдаёт источник.** Статусы на стороне платформ ни на
что не влияют и хранятся только для справки (`domains.external_status`). Домен, пропавший
из источника, перестаёт проверяться;
вернётся — снова начнёт проверяться.
...
проверяться. Домен, добавленный вручную через `/add`, забирает первый источник, который
...
следующий начнётся после него. Время в `/jobs` — по `DISPLAY_TIMEZONE` (по умолчанию
Europe/Moscow, UTC+3).
...
| `/jobs` | расписание: как часто и когда следующий запуск |
...
Время в `/jobs` — в часовом поясе `DISPLAY_TIMEZONE` (по умолчанию UTC+3).
