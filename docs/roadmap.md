...
- [x] Команды бота: `/status /list /check /scan_now /sync_now`
...
- [x] **Проверяется всё, что отдаёт источник** — статус платформы не решает, проверять
      домен или нет, но бан в PWApartners (status 9) сам по себе даёт вердикт `зашкварен`
      (checker `source_status`);
