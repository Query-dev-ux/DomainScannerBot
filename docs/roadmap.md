...
- Не упрёмся ли в app-level rate limit Graph API при ~200 доменах × несколько
  сканов в сутки.
- Формат ключа UClient: подтвердить на реальном ключе, что он передаётся логином
  Basic-auth с пустым паролем (Yii2 `HttpBasicAuth` по умолчанию) — иначе
  заполнить `UCLIENT_API_PASSWORD`.
